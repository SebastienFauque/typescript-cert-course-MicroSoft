...
let sortDescending: compare = (a, b) => {
    if (a > b) {
        return -1;
    } else if (b > a) {
        return 1;
    ...
    type numArraySort = (items: number, sortOrder: 'ascending' | 'descending') => number[];
    
    let buildArray: numArraySort = (items, sortOrder) => {
        let randomNumbers = [];
        let nextNumber;
    ...
    interface CalcLoan {
        (principle: number, interestRate: number, months: number): string;
    }

    let loanCalculator: CalcLoan = (principle, interestRate, months) => {
